...
<main>
    ...
    <div class="box">
        ...
        <div class="element">
            <p>Créez un groupe :</p>
            <form method="post" onsubmit="return confirm('Voulez vous vraiment créer un groupe ?')">
                <label for="mdpAtu" class="perso">Saisissez le nom du groupe à créer :</label>
                <div>
                    <input type="text" id="mdpAtu" name="txtNomGrp" required>
                </div>
                <label for="mdp" class="perso">Saisissez le mot de passe du groupe :</label>
                <div>
                    <input type="password" id="mdp" name="txtMdpGrp" required>
                </div>
                <label for="mdpConfirm" class="perso">Ressaisissez le mot de passe du groupe :</label>
                <div>
                    <input type="password" id="mdpConfirm" name="txtMdpConfirmGrp" required>
                </div>
                <div>
                    <button type="submit">Créer</button>
                    <input type="hidden" name="action" value="formulaireCreerGrp">
                </div>
            </form>
        </div>
        <hr>
        <?php if ($nbGrp[0]['COUNT(*)'] != 0) : ?>
            <div class="element">
                <p>Éditez vos groupes :</p><label class="perso">Cliquez sur le nom du groupe pour accéder à sa page de
                    personalisation.</label>
            </div>
            <?php foreach ($groupes as $groupe) : ?>
                <div class="element">
                    <label class="perso">Groupe :</label><a
                            href="?url=groupeAdmin&id=<?= $groupe->getNomGroupe() ?>&chef=<?= $groupe->getPseudoChef() ?>"><?= $groupe->getNomGroupe() ?></a>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
