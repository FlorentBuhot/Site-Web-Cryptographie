...
<?php $this->_t = "Vous avez dit sûr, ...sûr ?"; ?>
    ...
    <div class="element_section">
        ...
        <div id="cont_eni">
            ...
            <div class="main_box">
                ...
                <br>
                <br>
                <p align="justify">Il existe une méthode de chiffrement extrêmement sûre pour échanger entre deux
                    personnes un nombre secret M (par exemple 8). Elles choisissent simplement
                    ensemble à l'avance un nombre K (disons 5) qui leur servira de clé secrète partagée.<br>
                    Le chiffrement, de M consiste à calculer le nombre C = M + K (c'est-à-dire 13),
                    et le déchiffrement de C s'effectue en calculant C - K = (M + K) - K (et le résultat est bien
                    8).<br>
                    Ne connaissant pas K, un intrus qui intercepterait C n'aurait aucun
                    moyen de découvrir M. Dans la suite de cette énigme, le message M chiffré avec la clé K (appelé C
                    ci-dessus) est noté {M}k.</p>
                <br>
                <br>
                <p>
                    <span id="contenuePage"></span>
                </p>
    ...
    <?php if (isset($user)): ?>
        <div class="main_box">
            ...
            <button id='btnIndice' class="btn-ind" type="button" onclick="sendPoints();" form="formReponse">Découvrir un
                nouvel indice<br> (-<span id="points">375</span> points)
            </button>
        </div>
    <?php endif; ?>
...
<form method="post" id="formReponse">
    ...
    <input type="hidden" name="enigme" value="sursur">
...
<script type="text/javascript" src="<?= $dir . $enigme['OTP'] ?>"></script>
